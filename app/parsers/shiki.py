...
bearer_headers = {
    "User-Agent": "Anime4Mood", # Обязательно указать название приложения
    "Authorization": "Bearer ", # 'Bearer auth-token' приложения
}
...
async def request_shiki(url, params=None):
    """Получение Json от Shikimori

    Args:
        url (_type_): Ссылка, к которой будет производиться запрос
        params (_type_, optional): Параметры запрос. Defaults to None.
...
async def fetch_anime_list(page: int) -> list:
    """Получение списка аниме по странице пагинации
    ...
    params = {"page": page, "limit": 50, "order": "id"}
...
async def request_shiki_anime_list(session: AsyncSession):
    ...
    """
    page = 93 # Указать с какой страницы будут запросы к апи
    while True:
        ...
        for anime in anime_list:
            ...
            if len(extended_info["screenshots"]) > 0:
                await create_screenshot_item(
                    anime_info=extended_info, anime=anime_item, session=session
                )
...
async def create_anime_item(anime_info: dict, session: AsyncSession):
    anime_existence = (
        (await session.execute(select(Anime).where(Anime.shiki_id == anime_info["id"])))
        .scalars()
        .first()
    )
    if anime_existence:
    ...
    anime = Anime(
        ...
        started=datetime.strptime(anime_info["aired_on"], "%Y-%m-%d").date()
        if anime_info["aired_on"]
        else None,
        ended=datetime.strptime(anime_info["released_on"], "%Y-%m-%d").date()
...
async def create_screenshot_item(anime, anime_info, session: AsyncSession):
    for screen in anime_info["screenshots"]:
        screenshot_item = Screenshot(
            preview=f'https://shikimori.one{screen['preview']}',
...
async def create_image_item(anime, anime_info, session: AsyncSession):
    image_item = Image(
        preview=f'https://shikimori.one{anime_info['image']['preview']}',
